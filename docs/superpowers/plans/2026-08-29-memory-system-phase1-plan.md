...
src/memweave/
  ...
  storage/
    ...
    coordinator.py         # in-process projection dispatch (durable delivery is Outbox)
...
- `EventRepository.append(stream_id, event_type, payload, actor, request_id, event_id=None, occurred_at=None, causation_id=None, correlation_id=None, idempotency_key=None, protocol_version="1.0") -> Event`; `protocol_version` accepts a protocol version string or `ProtocolVersion` and is persisted on the immutable event. `list_after(stream_id, seq) -> list[Event]`; `last_seq(stream_id) -> int`; `find_existing(stream_id, event_id=None, idempotency_key=None) -> Event | None` for idempotency-aware command retries.
...
**Interfaces:**
- `SessionStore(database, ..., tenant_id=None)` exposes `apply_event(event: Event) -> SessionState`, `get(session_id) -> SessionState`, `upsert_active(memory) -> None`, database-backed `command_lease()` for cross-process command ordering, and `lease_release_errors()` for non-masking cleanup diagnostics. Its `session_states` and `session_command_leases` tables are provisioned by migrations `0004_session_states`, `0005_session_command_leases`, `0006_session_stream_identity`, and `0007_session_stream_recovery`. A tenant-scoped instance requires slash-delimited streams such as `tenant:<tenant_id>/session:<session_id>` or `tenant:<tenant_id>/project:<project_id>/session:<session_id>` and stores an isolated namespace; an unscoped instance accepts only `session:<session_id>`. SessionStore still accepts only continuous event watermarks, with gaps handled by ProjectionDispatcher/ProjectionRuntime.
- `SessionCommandCoordinator.append_explicit(operation, *, stream_id, actor, request_id, ...) -> SessionCommandResult`; Coordinator 先追平已提交事件并执行确定性操作预检，再由 EventStore 提交 `memory.command`，最后同步投影到 SessionStore；已存在的幂等事件跳过预检并重放原事件。
