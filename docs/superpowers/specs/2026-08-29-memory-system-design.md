...
“记忆网络”是 MemWeave 的逻辑总称，不要求所有对象存放在同一张表或同一个图数据库中。物理实现可以由多个可重建的存储和投影组成，但对象职责必须保持分离：

| 对象 | 含义 | 权威性与主要消费者 |
| --- | --- | --- |
| `Event` | 实际发生的输入、输出、工具结果或外部事件 | 不可变事实源；所有派生对象的证据 |
| `WorkingMemory` | 当前会话或任务暂时需要的状态 | Session Projection；当前 Agent 上下文 |
| `Memory` | 可跨轮次复用的事实、偏好、约束或决策 | Durable Projection；召回服务 |
| `Relation` | 节点之间的来源、支持、冲突、适用或使用关系 | 可重建派生关系；关系/图检索 |
| `Episode` | 一次任务从开始到结束的完整轨迹 | 经验提取器；评估器 |
| `Experience` | 从一个或多个 Episode 归纳并验证的规律 | 经验召回；Skill 生成候选 |
| `Skill` / `Workflow` | 带条件、步骤、工具和验证要求的程序性记忆 | Agent Runtime 执行；Registry 治理版本 |
| `Prediction` | 基于当前状态对意图或下一步的临时推断 | 仅作建议或预取依据，不是事实记忆 |
| `Policy` | 控制提取、召回、晋升和执行边界的规则 | Policy/Control Plane；不可由普通记忆绕过 |

所有高层对象必须保留来源事件、适用作用域、版本、状态和生成器信息。`Memory`、`Experience`、
`Skill` 和 `Prediction` 不得仅凭文本相似度互相转换。MemWeave 可以保存、检索和治理 `Skill`，
但实际执行由 Agent Runtime 负责；MemWeave 不直接执行任意技能代码，也不直接修改 Agent Prompt、
工具、模型或生产代码。
...
本节是实施路线概览，用于划定阶段范围；具体的一致性、安全、作用域和验收约束见后续章节。可执行的阶段目标、依赖和门槛维护在[总体路线计划](../plans/2026-09-memory-system-roadmap.md)中。
...
1. 普通事实/偏好候选提取、持久化和晋升策略。
2. 记忆关系基础模型、证据关联和冲突候选。
3. 向量索引适配器、关键词/向量混合检索和权威回查。
4. L2 API Proxy Adapter 和更多宿主框架适配器。
5. 用户确认、审计、敏感策略和提取/召回评测。

### 第三阶段：记忆关联、经验和 Skill

1. Episode 生命周期、任务轨迹和结果反馈。
2. 历史经验、失败模式和适用条件归纳。
3. Skill/Workflow Registry、版本、验证、复用和废弃。
4. 图索引插件、多跳召回、多租户权限和外部 Source Adapter。
5. 水位监控、积压保护、索引重建和经验网络运维能力。

### 第四阶段：预测与受控进化

1. 用户意图、下一步工作和风险预测；预测只作为建议或预取信号。
2. 召回、提取、工具和 Skill 策略的离线回放、影子运行和灰度。
3. Evaluator、Policy Registry、Experiment Registry 和可回滚发布。
4. Prompt、工具、模型和生产代码的变更由独立控制平面管理，不能由 MemWeave Core 直接执行。
