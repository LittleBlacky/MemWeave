...
- 发现 `tenant_id` 原先只拒绝 `:`，允许 `/` 会生成无法被自身 stream 解析器接受的租户
  stream；实例虽能创建，但所有投影和读取都会失败。
...
## 拒绝冲突的记忆 key 与 memory_id

日期：2026-09-04

- 发现 `FORGET` 同时提供 `key` 和 `memory_id` 时，低层定位逻辑优先使用 key，
  若两者指向不同活动记忆会误删 key 对应记录。
- 定位逻辑现在分别解析两个身份；只要两者同时提供且索引不一致（包括一方不存在），
  就拒绝操作，不修改 session 状态。仅提供其中一个身份的兼容行为保持不变。
- 新增冲突身份回归测试，验证事务失败后两条活动记忆都保留。

## 事件追加前拒绝确定性无效命令

日期：2026-09-04

- 发现对不存在记忆执行 `UPDATE` 等确定性失败会先生成 `memory.command` 事件，
  投影失败后该事件永久占据序号，导致后续事件因缺口无法投影，重放也会重复失败。
- `SessionStore.validate_operation()` 新增只读预检，在 Coordinator 持有会话租约时对
  当前快照执行深拷贝模拟；无效操作在 EventStore.append 前拒绝，不制造毒事件。
- 事件已落库后若发生数据库/投影服务临时故障，仍保留事件并按原有重放语义恢复；
  预检只处理确定性业务校验，不替代事件先落库原则。
- 新增回归测试验证无效 UPDATE 不占用 seq，随后合法命令仍可从 seq=1 正常写入。

## 读取屏障以最终水位判定降级

日期：2026-09-04

- 发现 `SessionReadBarrier` 只依赖 `catch_up()` 是否抛异常；若实现静默返回但没有达到
  请求水位，结果会错误地带 `degraded=False`。
